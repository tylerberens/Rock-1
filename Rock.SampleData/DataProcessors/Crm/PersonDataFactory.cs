...
{
    /// <summary>
    /// Create sample data for Families and People.
    /// </summary>
    public partial class PersonDataFactory : SampleDataFactoryBase
    {
        ...
        private readonly string _SampleDataForeignKey = "CRM Sample Data";
        ...
        {
            this.CreateFromXmlDocumentFile( @"http://storage.rockrms.com/sampledata/sampledata_1_7_0.xml" );
        }
        ...
        public void CreateFromXmlDocumentFile( string xmlFileUrl )
        {
            var activity = this.GetCurrentTaskActivity();

            var fileName = Path.GetFileName( xmlFileUrl );

            var saveFile = Path.Combine( Path.GetTempPath(), fileName );

            activity.LogInformation( "Copying source file..." );
            activity.LogDebug( "Source file is \"{0}\".", xmlFileUrl );
            activity.LogDebug( "Destination file is \"{0}\".", saveFile );

            var success = DownloadFile( xmlFileUrl, saveFile );

            if ( !success )
            {
                activity.LogError( "Could not load file." );

                return;
            }

            activity.LogInformation( "Parsing source file..." );
            var xdoc = XDocument.Load( saveFile );

            this.ProcessXmlDocument( xdoc );
        }
        ...
        public void CreateFromXmlDocumentText( string xml, bool removeExistingItems = true )
        {
            var xdoc = XDocument.Parse( xml.ToStringSafe().Trim() );

            this.ProcessXmlDocument( xdoc, removeExistingItems );
        }
        ...
        /// </summary>
        /// <param name="sampleXmlFile"></param>
        private void ProcessXmlDocument( XDocument xdoc, bool removeExistingItems = true )
        {
            var activity = this.GetCurrentTaskActivity();
            ...
            try
            {
                ...
                if ( removeExistingItems )
                {
                    ...
                    {
                        //_stopwatch.Start();
                        activity.LogInformation( "Removing existing data" );
                        ...
                        activity.LogInformation( "data deleted" );
                ...
                rockContext.WrapTransaction( () =>
                {
                    ...
                    activity.LogInformation( "families added" );

                    AddRelationships( elemRelationships, rockContext );
                    activity.LogInformation( "relationships added" );

                    AddLocations( elemLocations, rockContext );
                    activity.LogInformation( "locations added" );

                    AddGroups( elemGroups, rockContext );
                    activity.LogInformation( "groups added" );

                    AddConnections( elemConnections, rockContext );
                    activity.LogInformation( "people connection requests added" );

                    AddFollowing( elemFollowing, rockContext );
                    activity.LogInformation( "people following added" );

                    AddToSecurityGroups( elemSecurityGroups, rockContext );
                    activity.LogInformation( "people added to security roles" );

                    AddRegistrationTemplates( elemRegistrationTemplates, rockContext );
                    activity.LogInformation( "registration templates added" );

                    AddRegistrationInstances( elemRegistrationInstances, rockContext );
                    activity.LogInformation( "registration instances added..." );

                    rockContext.ChangeTracker.DetectChanges();
                    rockContext.SaveChanges( disablePrePostProcessing: true );
                    activity.LogInformation( "...changes saved" );

                    // add logins, but only if we were supplied a password
                    if ( !string.IsNullOrWhiteSpace( this.Password ) )
                    {
                        AddPersonLogins( rockContext );
                        activity.LogInformation( "person logins added" );
                    }

                    // Add Person Notes
                    AddPersonNotes( elemFamilies, rockContext );
                    rockContext.SaveChanges( disablePrePostProcessing: true );
                    activity.LogInformation( "notes added" );

                    // Add Person Previous LastNames
                    AddPeoplesPreviousNames( elemFamilies, rockContext );
                    rockContext.SaveChanges( disablePrePostProcessing: true );
                    activity.LogInformation( "previous names added" );
                ...
                // done.
                activity.LogInformation( "done" );

                activity.LogInformation( _sb.ToString() );
        ...
        private void AddRegistrationTemplates( XElement elemRegistrationTemplates, RockContext rockContext )
        {
            ...
            // TODO: These items should be set as configuration parameters.
        ...
        public void AddPersonToGroup( Guid groupGuid, Guid personGuid, RockContext rockContext )
        {
            var groupService = new GroupService( rockContext );
            ...
            // Add each person as a member of the group
            //foreach ( var personGuid in personGuidList )
            //{
                int personId = _peopleDictionary[personGuid];

                // Don't add if already in the group...
                if ( group.Members.Where( p => p.PersonId == personId ).Count() > 0 )
                {
                    return;
                }
            ...
            foreach ( var elemFamily in families.Elements( "family" ) )
            {
                ...
                if ( family != null )
                {
                    ...
                    foreach ( var person in members.Select( m => m.Person ) )
                    {
                        ...
                        var interactionsQuery = interactionService.Queryable().Where( r => r.PersonAlias.PersonId == person.Id );

                        foreach ( var interaction in interactionsQuery )
                        {
                            interactionService.Delete( interaction );
                        }

                        // Delete Search Keys.
                        var searchService = new PersonSearchKeyService( rockContext );

                        var searchQuery = searchService.Queryable().Where( r => r.PersonAlias.PersonId == person.Id );
        ...
        private int? GetAdminPersonAliasId( RockContext dataContext )
        {
            //int adminPersonId = this.CurrentPersonAliasId.GetValueOrDefault();

            var lookup = new PersonLookup( dataContext );

            var adminPerson = lookup.GetAdminPersonOrThrow();

            return adminPerson.PrimaryAliasId;
        }
        ...
        {
            var adminPersonAliasId = GetAdminPersonAliasId( rockContext );
            ...
            for ( DateTime date = startingDate; date <= endDate; date = frequency == Frequency.weekly ? date.AddDays( 7 ) : frequency == Frequency.monthly ? date.AddMonths( 1 ) : endDate.AddDays( 1 ) )
            {
                ...
                else
                {
                    batch = new FinancialBatch
                    {
                        ...
                        CreatedByPersonAliasId = adminPersonAliasId
                    };
        ...
        private List<GroupMember> BuildFamilyMembersFromXml( XElement elemMembers, RockContext rockContext )
        {
            ...
            foreach ( var personElem in elemMembers.Elements( "person" ) )
            {
                ...
                {
                    person = new Person();
                    person.CreatedByPersonAliasId = adminPersonAliasId;
        ...
        #region ISampleDataChangeActionProcessor

        public override List<SampleDataChangeAction> GetActionList()
        {
            var actions = new List<SampleDataChangeAction>();

            AddNewChangeAction( actions, "Add Standard Sample Data", "Add the standard Rock sample data set for People and Families." );
        ...
        {
            actions.Add( new SampleDataChangeAction { Category = "CRM > People", Key = name, Name = name, Description = description, Action = action } );
        }
    ...
    #region Specific People

    public partial class PersonDataFactory
    {
        public void AddPersonHelenEvansAndDuplicates()
        {
...
";
            ...
            CreateFromXmlDocumentText( xmlFamily, removeExistingItems: true );

            // Get Person: Helena Evans
            var dataContext = new RockContext();

            var personService = new PersonService( dataContext );

            var person = personService.Get( PersonLookup.HelenaEvansPersonGuid );

            var personAliasId = person.PrimaryAliasId.GetValueOrDefault();

            dynamic assessmentResult = new { QuestionNumber = 1, Most = new string[] { "5", "5" }, Least = new string[] { "1", "3" } };
            var assessmentResultJson = @"
{''1'':{''Most'':[''S'',''Uncomfortable with change''],''Least'':[''I'',''Enjoys having fun'']},''2'':{''Most'':[''C'',''Tries to avoid mistakes''],''Least'':[''D'',''Not easily moved'']},''3'':{''Most'':[''C'',''Thinks through issues''],''Least'':[''D'',''Quickly shares thoughts'']},''4'':{''Most'':[''D'',''Makes decisions quickly''],''Least'':[''C'',''Calculates actions'']},''5'':{''Most'':[''D'',''Takes risks''],''Least'':[''C'',''Sees imperfections'']},''6'':{''Most'':[''D'',''Produces results''],''Least'':[''C'',''Struggles acknowledging abilities'']},''7'':{''Most'':[''S'',''Available to others''],''Least'':[''I'',''Full of energy'']},''8'':{''Most'':[''D'',''Fully committed to the goal''],''Least'':[''S'',''Does not speak up'']},''9'':{''Most'':[''D'',''Gives direction''],''Least'':[''C'',''Pays attention to details'']},''10'':{''Most'':[''I'',''Encourages others''],''Least'':[''S'',''Follows others'']},''11'':{''Most'':[''S'',''Concerned for others''],''Least'':[''C'',''Plays down abilities'']},''12'':{''Most'':[''D'',''Is self sufficient''],''Least'':[''S'',''Concerned for others'']},''13'':{''Most'':[''D'',''Pursues goals''],''Least'':[''C'',''Avoids danger'']},''14'':{''Most'':[''I'',''Enjoys life fully''],''Least'':[''D'',''Starts things'']},''15'':{''Most'':[''D'',''Perseveres obstacles''],''Least'':[''C'',''Thinks things through'']},''16'':{''Most'':[''D'',''Takes risks''],''Least'':[''C'',''Is analytical'']},''17'':{''Most'':[''I'',''Enjoys others''],''Least'':[''S'',''Patient listener'']},''18'':{''Most'':[''I'',''Full of energy''],''Least'':[''S'',''Doesn't challenge others'']},''19'':{''Most'':[''I'',''Freely shows feelings''],''Least'':[''D'',''Overcomes difficulties'']},''20'':{''Most'':[''C'',''Is concerned about the facts''],''Least'':[''I'',''Enjoys people'']},''21'':{''Most'':[''S'',''Is generous''],''Least'':[''C'',''Does things the right way'']},''22'':{''Most'':[''S'',''Like things the way they are''],''Least'':[''I'',''Acts without thinking'']},''23'':{''Most'':[''S'',''Gives grace to others''],''Least'':[''C'',''Systematically thinks through issues'']},''24'':{''Most'':[''C'',''Follows instructions precisely''],''Least'':[''I'',''Seeks out others'']},''25'':{''Most'':[''D'',''Pushes others''],''Least'':[''C'',''Thinks through problems'']},''26'':{''Most'':[''I'',''Enjoys having fun with others''],''Least'':[''D'',''Freely expresses opinions'']},''27'':{''Most'':[''D'',''Forcefully pursues goals''],''Least'':[''C'',''Carefully assesses risks'']},''28'':{''Most'':[''D'',''Drives to complete the goal''],''Least'':[''C'',''Critical of others'']},''29'':{''Most'':[''S'',''Readily follows others''],''Least'':[''D'',''Does not show weakness'']},''30'':{''Most'':[''C'',''Follows rules''],''Least'':[''S'',''Satisfied with circumstances'']}}
";
            AddPersonAssessment( dataContext, Rock.SystemGuid.AssessmentType.DISC.AsGuid(), personAliasId, assessmentResult.ToJson() );
        }

        private void AddPersonAssessment(RockContext dataContext, Guid assessmentTypeGuid, int personAliasId, string assessmentResultJson )
        {
            var assessmentService = new AssessmentService( dataContext );

            var rockContext = new RockContext();

            var assessmentType = new AssessmentTypeService( rockContext ).Get( assessmentTypeGuid );

            var assessment = new Assessment();

            assessment.AssessmentTypeId = assessmentType.Id;
            assessment.PersonAliasId = personAliasId;
            ...
            assessment.AssessmentResultData = new { Result = assessmentResultJson, TimeToTake = 800 }.ToJson();
